...
import torch
import math
from models.blocks import VSSBlock
...
class ICISPGen(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], N=128, M=320,
        ...
        self.m_down1 = [VSSBlock(N, config[0], layer_id=i)
                        for i in range(config[0])] + \
                       [ResidualBlockWithStride(N, N, stride=2)]
        self.m_down2 = [VSSBlock(N, config[1], layer_id=i)
                        for i in range(config[1])] + \
                       [ResidualBlockWithStride(N, N, stride=2)]
        self.m_down3 = [VSSBlock(N, config[2], layer_id=i)
                        for i in range(config[2])] + \
                       [conv3x3(N, M, stride=2)]

        self.m_up1 = [VSSBlock(N, config[3], layer_id=i)
                      for i in range(config[3])] + \
                     [ResidualBlockUpsample(N, N, 2)]
        self.m_up2 = [VSSBlock(N, config[4], layer_id=i)
                      for i in range(config[4])] + \
                     [ResidualBlockUpsample(N, N, 2)]
        self.m_up3 = [VSSBlock(N, config[5], layer_id=i)
        ...
        self.ha_down1 = [VSSBlock(N, config[0], layer_id=i)
                         for i in range(config[0])] + \
                        [conv3x3(N, 192, stride=2)]

        self.h_a = nn.Sequential(
            *[ResidualBlockWithStride(320, N, 2)] + \
             self.ha_down1
        )

        self.hs_up1 = [VSSBlock(N, config[3], layer_id=i)
                       for i in range(config[3])] + \
        ...
        self.hs_up2 = [VSSBlock(N, config[3], layer_id=i)
                       for i in range(config[3])] + \
                      [subpel_conv3x3(N, 320, 2)]
